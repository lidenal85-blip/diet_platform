...
>
> **Финальная ротация 2026-09-17 01:11 UTC (токен из R1 засветился в переписке):**
> Revoke в BotFather → новый токен (md5 `4dacdc3137`) установлен helper-скриптом через
> stdin (format-гейт → getMe-валидация → бэкап `.env.bak-rot-20260917011130` → sed →
> md5-гейт; ложный INSTALL-MISMATCH гейта — баг `cut\n`, файл записан верно, см.
> §4). Рестарт: PID 2036445 → 2040967, health ok, **0 Traceback**.
> Пробы: NEW getMe `ok:true` `@Fatboyandgirl_bot` «ПУХЛЯШ»; **chat-exposed токен
> `25c4e5c117` → 401** (мёртв). Локальный `.env` обновлён (md5 сверен). Контрольный
> E2E `--fast` на whimco: **PASS exit 0**, §20-8 инварианты целы. Монитор идентичности:
> ручной прогон после ротации — `identity OK` (01:16:09).

### Шаг R2 — USERBOT_RELAY_TOKEN: подсистема МЕРТВА, ротация НЕ нужна → вариант A выполнен 2026-09-17
...
**Owner decision:** **A — ВЫБРАНО И ВЫПОЛНЕНО (2026-09-17).**

> **Статус 2026-09-17: ВЫПОЛНЕНО И ВЕРИФИЦИРОВАНО.**
> Коммит `3e3afb0` (refactor(config): remove dead userbot relay token, R2-A), запушен на origin.

Что сделано:
- `building_blocks/config.py`: удалены поле `userbot_relay_token` и маппинг `USERBOT_RELAY_TOKEN`.
- `building_blocks/vault_integration.py`: ключ удалён из списка миграции `migrate_from_env`.
- `.env.example`: блок Userbot Relay удалён (локально и на сервере, md5 бит-в-бит с `3e3afb0`).
- Локальный `.env`: утёкшая строка `USERBOT_RELAY_TOKEN=…` удалена (телефон).
- Прод `.env`: проверен — строки уже не было (подчищена 16-го при ротации токена бота).
- Сервер: 3 файла доставлены из `3e3afb0` (md5 бит-в-бит), compileall OK, рестарт
  (PID 1832232 → 2036445, зомби-хрупкость штатно: stop-sigterm timeout → SIGKILL),
  startup-маркеры в app-логе на месте, 0 Traceback, `/health` ok.
- Тесты: 76 passed; 10 падений `test_vault_integration.py` — прекондишн
  (`vault_client_v2` отсутствует в окружении), подтверждены идентичными на baseline-стэше.
...
- [x] R2: вариант A выполнен 2026-09-17 — мёртвая переменная удалена из кода/шаблонов (коммит `3e3afb0`), утёкшая строка вычищена из локального `.env`; в прод `.env` строки уже не было.
...
- [x] Прод-бот работает на новом токене; `/health` ok; E2E на whimco зелёный.
- [x] `.env.bak-20260630181938` не трекается; `.gitignore` покрывает env-бэкапы (`.env.*`, проверено `check-ignore`).
...
- [x] Прод-пул Gemini не изменился (`AIzaSyB5...omeQ`), E2E с LLM на сервере PASS.
- [x] `git ls-files` не содержит ни одного файла с реальными секретами (проверено 2026-09-17).
- [x] TEAM_NOTES.md дополнен уроком: env-файлы (включая *.bak) никогда не коммитятся (пункт 12).
...
| 🆕 md5-гейт ротации ложно падает (INSTALL-MISMATCH) | ✅ Баг сравнения: `cut` отдаёт хвостовой `\n` → md5(token\n) ≠ md5(token); файл записан верно (подтверждено no-newline гейтом и рестартом). Урок: сверять отпечатки без хвостовых переводов строк |
